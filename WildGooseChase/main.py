...
def main():
    ...
    start_img = pygame.image.load(
        'buttons/start_button.png').convert_alpha()
    start_button = button.Button(
        SCREEN_WIDTH // 2 - start_img.get_width() // 2, 430, start_img, 1)

    # load try_again button
    restart_img = pygame.image.load(
        'buttons/restart_button.png').convert_alpha()
    ...
    quit_img = pygame.image.load(
        'buttons/quit_button.png').convert_alpha()
    quit_button = button.Button(650, 430, quit_img, 1)

    # load skins button
    skins_img = pygame.image.load(
        'buttons/skins_button.png').convert_alpha()
    skins_button = button.Button(
        SCREEN_WIDTH - 650 - skins_img.get_width(), 430, skins_img, 1)
    ...
    # load hard button
    hard_img = pygame.image.load(
        'buttons/hard_difficulty_button.png').convert_alpha()
    hard_button = button.Button(
        SCREEN_WIDTH // 2 - start_img.get_width() // 2,
        375, hard_img, 1)

    # load normal button
    normal_img = pygame.image.load(
        'buttons/normal_difficulty_button.png').convert_alpha()
    normal_button = button.Button(
        SCREEN_WIDTH // 2 - start_img.get_width() // 2,
        225,
        normal_img, 1)

    # load easy button
    easy_img = pygame.image.load(
        'buttons/easy_difficulty_button.png').convert_alpha()
    easy_button = button.Button(
        SCREEN_WIDTH // 2 - start_img.get_width() // 2,
        75, easy_img, 1)

    # load resume button
    resume_img = pygame.image.load(
        "buttons/resume_button.png").convert_alpha()
    resume_button = button.Button(50, 430, resume_img, 1)

    # load next button
    next_img = pygame.image.load(
        "buttons/next_button.png").convert_alpha()
    next_button = button.Button(
        SCREEN_WIDTH - next_img.get_width() - 5, 430, next_img, 1)

    # load city button
    city_img = pygame.image.load(
        "buttons/city_button.png").convert_alpha()
    city_button = button.Button(
        SCREEN_WIDTH // 9 * 4.5, 75, city_img, 1)

    # load desert button
    desert_img = pygame.image.load(
        "buttons/desert_button.png").convert_alpha()
    desert_button = button.Button(
        SCREEN_WIDTH // 9 * 2.75, 225, desert_img, 1)

    # load beach button
    beach_img = pygame.image.load(
        "buttons/beach_button.png").convert_alpha()
    beach_button = button.Button(
        SCREEN_WIDTH // 9 * 6.25, 225, beach_img, 1)

    # load fields button
    fields_img = pygame.image.load(
        "buttons/fields_button.png").convert_alpha()
    fields_button = button.Button(
        SCREEN_WIDTH // 9 * 1.25, 75, fields_img, 1)

    # load default skin
    default_img = pygame.transform.scale(
        pygame.image.load('sprites/d1.png').convert_alpha(), (250, 250))
    default_button = button.Button(
        75, 50, default_img, 1)

    # load flamingo skin
    flamingo_img = pygame.transform.scale(
        pygame.image.load('sprites/f1.png').convert_alpha(), (250, 250))
    flamingo_button = button.Button(275, 50, flamingo_img, 1)

    # load cow skin
    cow_img = pygame.transform.scale(
        pygame.image.load('sprites/cw1.png').convert_alpha(), (250, 250))
    cow_button = button.Button(475, 50, cow_img, 1)

    # load santa skin
    santa_img = pygame.transform.scale(
        pygame.image.load('sprites/s1.png').convert_alpha(), (250, 250))
    santa_button = button.Button(675, 50, santa_img, 1)

    # load bow skin
    bow_img = pygame.transform.scale(
        pygame.image.load('sprites/b1.png').convert_alpha(), (250, 250))
    bow_button = button.Button(150, 225, bow_img, 1)

    # load canada skin
    canada_img = pygame.transform.scale(
        pygame.image.load('sprites/cn1.png').convert_alpha(), (250, 250))
    canada_button = button.Button(375, 225, canada_img, 1)

    # load paint skin
    paint_img = pygame.transform.scale(
        pygame.image.load('sprites/p1.png').convert_alpha(), (250, 250))
    ...
    rectangle = pygame.Rect(SCREEN_WIDTH,
                            (SCREEN_HEIGHT // 2) -
                            spritesHandler.sprite.get_height(),
                            50, 50)
    ...
    while running:
        if not game_started:
            ...
            if menu == "title":
                ...
                backgroundsHandler.draw_background(scroll,
                                                   screen,
                                                   SCREEN_WIDTH)
                ...
                screen.blit(skins_img,
                            (SCREEN_WIDTH - 650 - skins_img.get_width(), 430))
                screen.blit(quit_img,
                            (650, 430))
                screen.blit(start_img,
                            (SCREEN_WIDTH // 2 -
                             start_img.get_width() // 2, 430))

            elif menu == "skins":
                backgroundsHandler.draw_background(
                    scroll, screen, SCREEN_WIDTH)
                weatherHandler.drawWeatherFront(screen)
                ...
                draw_text("Unlock New Skins With Points!",
                          font, TEXT_COL, 25, 10)
                ...
                draw_text("Default: 0p",
                          small_font, TEXT_COL, 75 + 50, 80)
                screen.blit(flamingo_img, (275, 50))
                draw_text("Flamingo: 10p",
                          small_font, TEXT_COL, 275 + 50, 80)
                screen.blit(cow_img, (475, 50))
                draw_text("Cow: 20p",
                          small_font, TEXT_COL, 475 + 70, 80)
                screen.blit(santa_img, (675, 50))
                draw_text("Santa: 30p",
                          small_font, TEXT_COL, 675 + 50, 80)
                screen.blit(bow_img, (150, 225))
                draw_text("Fancy: 40p",
                          small_font, TEXT_COL, 150 + 50, 265)
                screen.blit(canada_img, (375, 225))
                draw_text("Canada: 50p",
                          small_font, TEXT_COL, 375 + 50, 265)
                screen.blit(paint_img, (600, 225))
                draw_text("Painter: 60p",
                          small_font, TEXT_COL, 600 + 50, 265)
                ...
                screen.blit(easy_img, (SCREEN_WIDTH // 2 -
                                       start_img.get_width() // 2, 75))
                screen.blit(normal_img, (SCREEN_WIDTH // 2 -
                                         start_img.get_width() // 2, 225))
                screen.blit(hard_img, (SCREEN_WIDTH // 2 -
                                       start_img.get_width() // 2, 375))
                ...
                screen.blit(back_img, (5, 430))
                screen.blit(next_img,
                            (SCREEN_WIDTH - next_img.get_width() - 5, 430))
                ...
                screen.blit(back_img, (5, 430))
                screen.blit(next_img, (SCREEN_WIDTH -
                                       next_img.get_width() - 5, 430))
            ...
            camera_offset_x = screen.get_width() // 2 - image_x -\
                spritesHandler.sprite.get_width() // 2
            ...
            goose_hitbox = pygame.Rect(
                rectangle_x + 20, rectangle_y + 40, 80, 60)
            ...
            if len(obstacles) == 0:
                obstaclesHandler.randomize()
                if (obstaclesHandler.obstacle ==
                    obstaclesHandler.pigeonSprites or
                        obstaclesHandler.obstacle ==
                        obstaclesHandler.waspSprites or
                        obstaclesHandler.obstacle ==
                        obstaclesHandler.bluebirdSprites):
                    obstacles.append(HighOb(obstaclesHandler.obstacle_sprite,
                                            SCREEN_HEIGHT - 50))
                else:
                    obstacles.append(LowOb(obstaclesHandler.obstacle_sprite,
                                           SCREEN_HEIGHT - 175))
            ...
            for obstacle in obstacles:
                ...
                obstacle_hitbox = pygame.Rect(obstacle.rect.x + 25,
                                              obstacle.rect.y + 25,
                                              60,
                                              60)
            ...
            for collectable in collectables:
                ...
                collectable_hitbox = pygame.Rect(collectable.rect.x + 22,
                                                 collectable.rect.y + 22,
                                                 40,
                                                 40)
